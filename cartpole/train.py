...
def main(unused_argv):
    ...
    if not cluster or task.type == "master" or task.type == "worker":
        ...
        with sv.managed_session(target, config=config) as sess:
            ...
            observation = np.reshape(observation, [1, D])
            ...
            while episode_number <= total_episodes:
                ...
                action = model.get_action(sess, observation)

                # step the environment and get new measurements
                new_observation, reward, done, info = env.step(action)
                new_observation = np.reshape(new_observation, [1, D])

                # reward_sum += reward
                reward = reward if not done else -10

                model.after_action(sess, observation, action, reward, new_observation, done)
                ...
                if done:
                    ...
                    print("episode: {}/{}, score: {}, e: {:.2}"
                          .format(episode_number, total_episodes, time, model.epsilon))
                    ...
                    # if episode_number % batch_size == 0:
                        ...
                        # logging.info('Global step %d. Average reward for episode %f.  Total average reward %f.' % (
                        #     global_step_val, reward_sum / batch_size, running_reward / batch_size))
